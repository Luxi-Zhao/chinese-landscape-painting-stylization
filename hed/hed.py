...
def class_balanced_sigmoid_cross_entropy(logits, label, name='cross_entropy_loss'):
    ...
    with tf.name_scope('class_balanced_sigmoid_cross_entropy'):
        ...
        cost = tf.nn.weighted_cross_entropy_with_logits(
            logits=logits, labels=y, pos_weight=pos_weight)
...
class Model(ModelDesc):
    ...
    def build_graph(self, image, edgemap):
        ...
        wd_w = tf.compat.v1.train.exponential_decay(2e-4, get_global_step_var(),
                                          80000, 0.7, True)
...
def run(model_path, in_path, out_path, level):
    ...
    for f in files:
      ...
      for k in level:
          ...
          cv2.imwrite("{}/{}_{}.jpg".format(
            ...
            image_name, 
            str(k + 1)), pred * 255)
      # logger.info("Results saved to out*.jpg")


if __name__ == '__main__':
  parser = argparse.ArgumentParser()
  parser.add_argument('--load', help='load model')
  parser.add_argument('--inpath', help='image path')
  parser.add_argument('--outpath', help='output hed path')
  args = parser.parse_args()
  run(args.load, args.inpath, args.outpath, [2])
